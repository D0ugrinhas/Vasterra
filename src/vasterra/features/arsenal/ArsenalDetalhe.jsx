  ...
  return (
    <div style={{ maxWidth: 900 }}>
      <div style={{ display: "flex", gap: 16, marginBottom: 20 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: "'Cinzel',serif", fontSize: 22, color: G.gold2, marginBottom: 6 }}>{item.nome}</div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            {item.defesa && <Pill label={`DEF: ${item.defesa}`} cor="#4eb0ff" />}
          </div>
        </div>
      </div>
      ...
      <div style={{ background: G.bg2, border: "1px solid " + G.border, borderRadius: 10, padding: 12, marginBottom: 10, color: G.text }}>
        <div style={{ fontFamily: "monospace", fontSize: 11, color: G.muted, marginBottom: 8 }}>Preço em Vastos: <span style={{ color: G.gold }}>{fmtVastos(item.vastos || {})}</span></div>
