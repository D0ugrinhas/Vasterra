...
export function TabAtributos({ ficha, onUpdate, inventarioNomes = [], arsenal = [] }) {
  ...
  const attrMods = aggregateModifiers([...(ficha.modificadores?.atributos || []), ...itemMods], "atributos");
  const periciaModsParsed = [ ...(ficha.modificadores?.pericias || []), ...itemMods ];
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: 16 }}>
      ...
      <div style={{ background: G.bg2, border: "1px solid " + G.border, borderRadius: 10, padding: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, paddingBottom: 8, borderBottom: "1px solid " + G.border }}><span style={{ fontFamily: "'Cinzel',serif", fontSize: 12, color: G.gold, letterSpacing: 3 }}>◈ PERÍCIAS</span><HoverButton onClick={() => setModsOpen("pericias")} style={btnStyle({ padding: "2px 8px" })}>⚙</HoverButton></div>
